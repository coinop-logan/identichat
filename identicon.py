...
def interpolateFromPoints(p1, p2, ratio):
    assert(ratio <= 1 and ratio >= 0)

    p2p = [p2[0] - p1[0], p2[1] - p1[1]]
    return [p1[0] + ratio*p2p[0], p1[1] + ratio*p2p[1]]
...
def generateIdenticon(address, width, height):
    ...
    randNum1 = dataSource.consumeWordsIntoRange(1,0,1)
    randNum2 = dataSource.consumeWordsIntoRange(1,0,1)
    if (randNum2 < randNum1):
        [randNum1, randNum2] = [randNum2, randNum1]
    ratio0 = randNum1
    ratio1 = randNum2 - randNum1
    ratio2 = 1 - randNum2

    pupilPoint = addPoints(scalePoint(eyePoints[0], ratio0), scalePoint(eyePoints[1], ratio1), scalePoint(eyePoints[2], ratio2))
    pygame.draw.circle(halfFaceSurface, [255,0,0], integerizePoint(pupilPoint), 2)
    ...
    #---ear shape---
    earPoints = [None, None, None]

    #(21,22) Points where the ear connects to the forehead edge (earPoints[0] and earPoints[2])
    earPoint0Ratio = dataSource.consumeWordsIntoRange(1,0,1)
    earPoint1Ratio = dataSource.consumeWordsIntoRange(1,0,1)

    earPoints[0] = interpolateFromPoints(facePoints[2], facePoints[1], earPoint0Ratio)
    earPoints[2] = interpolateFromPoints(facePoints[2], facePoints[1], earPoint1Ratio)

    #(23,24) 'tip' pos of ear
    tipXRatio = dataSource.consumeWordsIntoRange(1, 0, 1)
    tipX = facePoints[2][0] + tipXRatio * (halfWidth - facePoints[2][0])

    tipYMax = tipXRatio * facePoints[1][1]
    tipY = dataSource.consumeWordsIntoRange(1, 0, tipYMax)

    earPoints[1] = [tipX, tipY]

    earColor = dataSource.consumeWordsIntoColor()

    pygame.draw.polygon(halfFaceSurface, earColor, earPoints)
    ...
    print(len(dataSource.hexStr), "words left unused.")
